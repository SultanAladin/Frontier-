...
public:
    ...
    // Editor picking: the cursor tap in RENDER pixels lands in the next recorded frame; the answer is read from
    //    the slot the GPU has finished with, like the luminance and telemetry readbacks (one-two frames stale,
    //    invisible to a click). kNoVisibilityPick when no tap landed or the tap missed geometry; otherwise the
    //    packed id whose top 18 bits are the instance ordinal (SceneRecords.slang).
    void                        AssignPickTap(uint32_t PixelX, uint32_t PixelY) noexcept { Visibility.AssignPickTap(PixelX, PixelY, true); }
    [[nodiscard]] uint32_t      QueryPickedVisibility() const noexcept;

    // Editor selection outline: which instances the next frame rims in green (the true silhouette off the id
    //    image — VisibilityExchange::RecordSelectionOutline). Count 0 turns the stroke off.
...
private:
    ...
    // Editor selection outline: the picked instance ordinals the next frame strokes (16 = kMaxEditorPicked).
    uint32_t                OutlinePicked[16] = {};
    uint32_t                OutlinePickedCount = 0u;

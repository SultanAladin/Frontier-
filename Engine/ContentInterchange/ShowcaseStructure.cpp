...
//      Y = +16.2  row 12  automotive flake paint: clear coat over a flake layer — lattice 26 → 9 cells/m, chip size
//                          0.30 → 0.46 of a cell, colour spread in thirds (aluminium | tinted | full candy)
    ...
    for (uint32_t Row = 0u; Row < kShowcaseGridSide; ++Row)
        for (uint32_t Col = 0u; Col < kShowcaseGridSide; ++Col)
        {
            ...
            case 11:  // matte ceramic / rubber, alternating: even columns bright satin ceramic, odd dark rubber
                ...
                break;
            case 12:  // M6b — AUTOMOTIVE METALLIC / CANDY FLAKE PAINT. The row used to be a bare density sweep over
                      //    hue-tinted metal, and at those frequencies every flake was sub-pixel: the parameters were
                      //    read but nothing was visible. It is now the material the flake model exists for — a dark
                      //    saturated basecoat under a clear coat, with a real flake layer between them:
                      //      · uv_scale is the LATTICE FREQUENCY [cells/m]: 190 → 70 across the row, so the left end
                      //        is a fine metallic and the right a coarse show-car flake (a 1.1 m sphere spans ~210 →
                      //        ~77 cells: a few pixels per chip in the close shot — a flake, not a painted disc);
                      //      · slate_glint_size is the chip's radius within its cell (0.26 → 0.40 of the cell);
                      //      · slate_glint_color_spread runs in THIRDS across the row, so one shot shows all three
                      //        pigments: cols 0-4 plain aluminium (0.0), 5-9 a tinted mix (0.55), 10-14 full
                      //        multi-colour candy flake (1.0) — the "flakes have different colours" material.
            {
                const float Third = static_cast<float>(Col) / 5.0f;   // 0,0,0,0,0 | 1,… | 2,…
                HueColor(Hue, 0.90f, 0.10f, S.BaseColor);             // dark saturated basecoat: flakes must out-read it
                S.BaseMetalness = 0.0f;
                S.SpecularRoughness = 0.45f;                          // matte paint around a polished chip
                S.SlateGlintDensity = 6.0f;                           // coverage 1 − e^(−6/4) ≈ 78 % of cells hold a flake
                S.SlateGlintUvScale = 190.0f - 120.0f * t;            // [cells/m] fine metallic → coarse show flake
                S.SlateGlintSize = 0.26f + 0.14f * t;
                S.SlateGlintColorSpread = Third < 1.0f ? 0.0f : (Third < 2.0f ? 0.55f : 1.0f);
                S.CoatWeight = 1.0f; S.CoatRoughness = 0.04f; S.CoatIor = 1.6f;   // the clear coat the flakes sit under
                break;
            }

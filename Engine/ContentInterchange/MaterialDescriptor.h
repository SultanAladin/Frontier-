...
struct MaterialSlabDescriptor
{
    ...
    // M6b: the two properties that make a flake a flake rather than a noise field. They ride the slab record's two
    //    trailing reserved lanes (Slate2.zw), so NOTHING in the 58-float prefix or any pinned offset moves.
    float SlateGlintSize                 = 0.25f;                 // [-]   flake radius as a fraction of its lattice cell (0.02 … 0.5)
    float SlateGlintColorSpread          = 0.0f;                  // [-]   0 = plain aluminium flakes; 1 = a full pigment rainbow per flake

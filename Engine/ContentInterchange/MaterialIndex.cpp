...
MaterialSlabDescriptor FoldVertical(const MaterialSlabDescriptor& Top, const MaterialSlabDescriptor& Bottom) noexcept
{
    ...
    if (Top.SlateGlintDensity > 0.0f && Out.SlateGlintDensity == 0.0f)
    {
        ...
        Out.SlateGlintSize = Top.SlateGlintSize; Out.SlateGlintColorSpread = Top.SlateGlintColorSpread;   // M6b: size/colour ride the pair down
    }
...
MaterialSlabRecord MaterialIndex::ConstructSlabRecord(const MaterialSlabDescriptor& S, float MixWeight) noexcept
{
    ...
    R.GlintSize           = S.SlateGlintSize;          // M6b: the two lanes that used to be Reserved0/1
    R.GlintColorSpread    = S.SlateGlintColorSpread;
    return R;
}

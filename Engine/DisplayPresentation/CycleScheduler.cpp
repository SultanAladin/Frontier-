...
bool CycleScheduler::Initialize() noexcept
{
    ...
    // Layer 6
    if (!WorkspaceHostUnit.Initialize(Config.ViewportWidth, Config.ViewportHeight))
    {
        return false;
    }
...
void CycleScheduler::ExecuteCycle(float WallClockDeltaSeconds) noexcept
{
    ...
    // ⑬ [WorkspaceHost Present]: Immediate mode overlay & offscreen target presentation
    WorkspaceHostUnit.RenderWorkspaceOverlay(WorkspaceHostUnit.QueryEditMode(), WallClockDeltaSeconds);

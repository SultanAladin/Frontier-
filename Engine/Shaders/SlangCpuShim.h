...
// M6b: the flake lattice quantises world position into cells — GLSL's floor/fract, spelled once so the shader text
//    and the CPU port land on the SAME cell for the same point (a flake that moves between the two is not a mirror).
#ifndef _MSC_VER
inline float floor(float x) { return std::floor(x); }
#endif
inline vec3  floor(vec3 x) { return vec3(std::floor(x.x), std::floor(x.y), std::floor(x.z)); }
inline float fract(float x) { return x - std::floor(x); }
inline vec3  fract(vec3 x) { return vec3(fract(x.x), fract(x.y), fract(x.z)); }

//============================================================================================================================================
// 📦 Frontier/Layer2_VolumetricDynamics/ParticleIntegrator.cpp — Particle Integration Implementation
//============================================================================================================================================
...
ParticleIntegrator::ParticleIntegrator(size_t MaximumParticleCapacity) noexcept
    : m_Capacity(MaximumParticleCapacity)
    , m_Particles(MaximumParticleCapacity)
    , m_ActiveCount(0)
{
}
...
void ParticleIntegrator::EmitParticles(const EmitterConfiguration& Emitter, uint32_t Count) noexcept
{
    for (uint32_t i = 0; i < Count; ++i)
    {
        ...
        ParticleRecord& p = m_Particles[m_ActiveCount++];
...
Vector3 ParticleIntegrator::EvaluateCurlNoise(const Vector3& Position) const noexcept
{
...
void ParticleIntegrator::AdvanceSimulation(float Δτ, const FluidSolver* Fluid) noexcept
{
    ...
    for (size_t i = 0; i < m_ActiveCount; ++i)
    {
        ...
        if (p.IsAlive())
        {
            ...
            Vector3 Curl = EvaluateCurlNoise(p.SpatialLocation);
            ...
            if (AliveCursor != i)
            {
                m_Particles[AliveCursor] = p;
            }
            ++AliveCursor;
        }
    }
    m_ActiveCount = AliveCursor;
}

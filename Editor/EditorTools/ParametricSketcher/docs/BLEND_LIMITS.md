...
Phase 32n scales the same construction to `3 <= N <= 8`. Canonical source topology is `V=8+2N`, `E=12+3N`, `C=24+6N`, `L=6+3N`, `F=6+2N`; the rounded result is `V=16+2N`, `E=24+3N`, `C=48+6N`, `L=10+3N`, `F=10+2N`, with genus zero throughout. Both ends, safe offsets, deterministic order, rigid transforms, and all `N(N-1)/2` finite-cylinder clearance checks are supported. Oblique, intersecting, more-than-eight, non-cylindrical, or multiple side-entering blind cavities refuse transactionally; one orthogonal side cavity is handled separately by Phase 32r.

...
function mkEl(id){return {id,style:{setProperty(){}},classList:{add(){},remove(){},toggle(){},contains(){return false}},dataset:{},innerHTML:'',textContent:'',value:'',
  ...
  getContext(){return new Proxy({},{get:(t,k)=>k==='measureText'?()=>({width:10}):(k==='createRadialGradient'||k==='createLinearGradient')?()=>({addColorStop(){}}):()=>{}})},width:0,height:0,parentElement:null};}
...
{ if(M.anyTool&&M.anyTool())M.selectTool();M.view.target=[0,0,0];M.view.dist=260;M.setView('top');M.resize();
  ...
  ok(b.faceOps&&b.faceOps.length===1&&b.faceOps[0].op==='push'&&b.faceOps[0].h===5,'push 5 mm recorded as a face feature');let m=M.meshOf(b);ok(Math.abs(M.meshVolume(m)-v0*25/20)<1e-6*v0&&openEdges(m)===0&&m.brep.faces.length===6,'pushing the whole cap extends the walls: still 6 faces, volume ×1.25, watertight');
  ...
  ok(b.faceOps.length===2&&b.faceOps[1].op==='inset'&&m.brep.faces.some(f=>f.key.startsWith('inset|cap:top'))&&openEdges(m)===0,'inset 3 mm adds a ring face, cap:top keeps its key, watertight');
  M.sub_.sel.set('k',{fig:b.id,kind:'face',idx:top()});M.startOp('extrude');M.solidKey({key:'-'});M.solidKey({key:'6'});M.solidKey({key:'Enter'});m=M.meshOf(b);ok(b.faceOps.length===3&&b.faceOps[2].h===-6&&M.meshVolume(m)<v0*1.25&&openEdges(m)===0&&m.brep.faces.length===8,'pocket: pushing the inset face −6 mm cuts in (4 pocket walls), watertight');
  // gizmo: moving a face = push along its normal
  const before=M.meshVolume(m);M.sub_.sel.clear();M.sub_.sel.set('k',{fig:b.id,kind:'face',idx:top()});const st=M.subBegin();ok(st&&st.faces&&st.faces.length===1,'subBegin captures the selected B-rep face');M.subApply(st,[0,0,4]);M.subEnd();m=M.meshOf(b);ok(b.faceOps.length===4&&Math.abs(b.faceOps[3].h-4)<1e-9&&M.meshVolume(m)>before&&openEdges(m)===0,'moving a face with the gizmo pushes it along its normal (live, undoable)');
  M.undo();ok((M.byId(b.id).faceOps||[]).length===3,'undo removes the face move');

...
const mE=M.meshOf(ex);ok(mE.tris.length===12&&mE.lines.length>0,'extrude mesh: 4 walls (8 tris) + 2 caps (4 tris)');const ms=M.measure(ex);ok(ms.lab==='volume'&&Math.abs(ms.v-Math.abs(M.polyArea(M.curvePlane(rE).pts))*25)<1e-6,'volume = profile area × height');

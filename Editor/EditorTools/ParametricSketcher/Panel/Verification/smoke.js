...
M.resize();
// 0. imported/native documents are bounded and validated before touching live state
let rejected=false;try{M.validateDocumentPayload({app:'Other',figures:[]});}catch(e){rejected=true;}ok(rejected,'rejects a document for another application');
rejected=false;try{M.validateDocumentPayload({app:'SolidArc',figures:[{id:1,kind:'body',name:'<img src=x onerror=alert(1)>',params:{}}]});}catch(e){rejected=true;}ok(rejected,'rejects markup in imported document strings');
rejected=false;try{M.validateDocumentPayload({app:'SolidArc',figures:[{id:1,kind:'body',name:'A',params:{}},{id:1,kind:'body',name:'B',params:{}}]});}catch(e){rejected=true;}ok(rejected,'rejects duplicate figure ids');

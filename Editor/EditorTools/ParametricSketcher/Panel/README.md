...
- Cap-edge fillets are **Blender-style**: only the picked edge is bevelled; a neighbouring corner-fillet arc is not rolled into — the band terminates against it with a mitre / triangular blend. Offset rings are miter-limited and tangent joints are handled analytically, so a fillet next to an existing vertical-edge fillet cannot spike or fly off. Tangent (fillet-boundary) edges are not valid fillet targets.

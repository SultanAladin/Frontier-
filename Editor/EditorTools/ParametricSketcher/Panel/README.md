...
- Cap-edge fillets follow the **rolling-ball rule**: the fillet propagates along the tangent-continuous chain (through profile-corner fillet arcs and smooth curves) and stops at sharp corners; offset rings are miter-limited and tangent joints are handled analytically, so a fillet next to an existing vertical-edge fillet cannot spike or fly off. Tangent (fillet-boundary) edges are not valid fillet targets.

...
The browser/native boundary is now explicit: [`solidarc-bridge.js`](solidarc-bridge.js) supports WebView2, WKWebView, generic embedded hosts, and an opt-in development WebSocket while preserving offline prototype mode. See [`INTEGRATION.md`](INTEGRATION.md) for the versioned message contract and C++ ownership rules.

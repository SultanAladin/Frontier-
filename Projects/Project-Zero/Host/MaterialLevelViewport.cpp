...
// InterfaceSignedDistance.slang needs GLSL's two-argument atan (C's atan2); fract/floor now come from the shared
//    shim (M6b moved them there — the flake lattice quantises with them and both mirrors must use one definition).
...
std::vector<float>             g_MatGlintSize;          // M6b: Slate2.z — flake radius / lattice cell
std::vector<float>             g_MatGlintColorSpread;   // M6b: Slate2.w — per-flake pigment spread
...
// M6b — the kernel's glint block (ReSTIRViewport.slang, ResolveMaterial), line for line, in ONE place so the primary
//    surface and every bounce cannot drift apart: the lattice sample, the facet normal, and the four channel
//    modulations (pigment into base + specular colour, metalness to 1, roughness to the polished 0.08). Returns the
//    perturbed shading normal; `M` is modulated in place. `Ng` supplies the same below-surface rejection the map obeys.
vec3 ApplyGlintFlakes(int Material, const vec3& P, vec3 Ns, const vec3& Ng, ShadingRecord& M)
{
    if (g_MatGlintsOff || Material < 0 || static_cast<size_t>(Material) >= g_MatGlintDensity.size()) return Ns;
    if (g_MatGlintDensity[static_cast<size_t>(Material)] <= 0.0f) return Ns;
    const size_t I = static_cast<size_t>(Material);
    const float Coverage = 1.0f - expf(-g_MatGlintDensity[I] * 0.25f);
    const float Cells    = max(g_MatGlintUvScale[I], 0.01f);
    const vec4  Flake    = SlateFlakeSample(P, Cells, Coverage, g_MatGlintSize[I], g_MatGlintColorSpread[I]);
    if (Flake.x <= 0.0f) return Ns;
    const vec3 Gn = SlateFlakeNormal(Ns, P, Cells, Flake.x, 0.35f);
    if (dot(Gn, Ng) > 0.0f) Ns = Gn;
    const vec3 Tint = vec3(Flake.y, Flake.z, Flake.w);
    M.BaseColor         = mix(M.BaseColor, Tint * 0.92f, Flake.x);   // the chip REPLACES the paint, never tints it
    M.SpecularColor     = mix(M.SpecularColor, Tint, Flake.x);
    M.Metalness         = mix(M.Metalness, 1.0f, Flake.x);
    M.SpecularRoughness = mix(M.SpecularRoughness, 0.16f, Flake.x);
    return Ns;
}
...
Viewpoint ShowcaseViewpointFor(const std::string& Name)
{
    ...
    // M6b: the flake ROW, framed from the gap between rows 11 and 12 (Y = 15.15) so row 11 is behind the camera and
    //    row 12 fills the plate — the old eye at Y = 12.7 put two rows of other materials in front of the subject.
    if (Name == "glints") return { Frontier::Vector3{  0.0f, 14.95f, 1.55f }, -14.0f,  0.0f, 64.0f };
    // M6b — the flake close-ups. Row 12 sits at Y = +16.2 behind row 11 (Y = +14.7), so the shot looks DOWN over
    //    row 11's crown (its top is z = 1.10; the sight line clears it at z ≈ 1.7) from 3.3 m out. One view per
    //    colour-spread third: col 2 = plain aluminium, col 7 = tinted mix, col 12 = full candy flake.
    //    The eye sits BETWEEN rows 11 and 12 (Y = 15.15, the 1.5 m gap's midpoint) at z = 1.05, pitched −12° onto
    //    the sphere's equator 1.05 m ahead: nothing of row 11 is in front of it, and a 34° FoV fills the frame with
    //    one 1.1 m sphere. This is the lookdev shot — the flakes are what the frame is for.
    if (Name == "flakes-alu")   return { Frontier::Vector3{ -7.5f, 15.15f, 1.05f }, -12.0f, 0.0f, 34.0f };  // col 2  — spread 0.00, plain aluminium
    if (Name == "flakes")       return { Frontier::Vector3{  0.0f, 15.15f, 1.05f }, -12.0f, 0.0f, 34.0f };  // col 7  — spread 0.55, tinted mix
    if (Name == "flakes-candy") return { Frontier::Vector3{  7.5f, 15.15f, 1.05f }, -12.0f, 0.0f, 34.0f };  // col 12 — spread 1.00, full candy
              ...
              float InitialLastPdf = 0.0f, vec3 InitialBeta = vec3(1.0f))
{
    ...
    for (int Depth = 0; Depth < Bounces + 4; ++Depth)
    {
        ...
        ShadingRecord m = Mat;                                  // local copy: the glint block and the nested
                                                                //    fallback both modulate it
        // M6b glints — the same flakes the primary surface takes; a bounce that lands on a glinting material must
        //    see the same microsurface AND the same pigment, or the two paths shade two different materials.
        Ns = ApplyGlintFlakes(T.Material, P, Ns, Ng, m);
        vec3 Tt, Bt;
...
bool BuildInterfacePanel(uint16_t ObjectId)
{
    ...
    for (const MaterialRecord& R : Records)
    {
        ...
        g_MatGlintSize.push_back(S.GlintSize);              // M6b: Slate2.zw
        g_MatGlintColorSpread.push_back(S.GlintColorSpread);
    }
...
bool BuildLevel()
{
    ...
    // ⚠️ M6b FIX — the M6 slice seated the glint arrays in the interface-panel path ONLY, so on the level itself every
    //    g_MatGlintDensity[Material] read past the end of an EMPTY vector: undefined behaviour that happened not to
    //    crash, and whatever it read is what the first glint sheet actually rendered. The level's own materials are
    //    seated here, with the records they came from, before any shading runs.
    g_MatGlintDensity.assign(Records.size(), 0.0f);
    g_MatGlintUvScale.assign(Records.size(), 1.0f);
    g_MatGlintSize.assign(Records.size(), 0.25f);
    g_MatGlintColorSpread.assign(Records.size(), 0.0f);
    for (size_t I = 0; I < Records.size(); ++I)
    {
        ...
        g_MatGlintDensity[I] = S.SlateGlintDensity;       // M6: P14.xy
        g_MatGlintUvScale[I] = S.SlateGlintUvScale;
        g_MatGlintSize[I] = S.GlintSize;                  // M6b: Slate2.zw
        g_MatGlintColorSpread[I] = S.GlintColorSpread;
    }
        ...
        RunRows(0, [&](int X, int Y, FrameTally& T)
        {
            ...
            Surf = RestirSurface();
            {
                ...
                if (H.Valid)
                {
                    ...
                    // M6b glints — the kernel's block through the shared helper (lattice sample, facet normal,
                    //    pigment into base/specular, metalness, polished roughness), applied before the frame builds.
                    Surf.Ns = ApplyGlintFlakes(Tri.Material, Surf.P, Surf.Ns, Surf.Ng, Surf.Mat);

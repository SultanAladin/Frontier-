...
void main()
{
    ...
    if (Enabled == 0u || CentreSurface.w <= 0.0)
    {
        ...
        if (FinalLevel != 0u) { float D = DitherOffset(Pixel); imageStore(OutputImage, Pixel, vec4(ToneMap(Centre.rgb * imageLoad(AlbedoImage, Pixel).rgb) + vec3(D, D, D), 1.0)); }   // V7+V5
        return;
    }
    ...
    // V6: the screen-space depth gradient, from the immediate neighbours (4 extra surface loads per pixel against
    ...
    // V4: the variance estimate is linear-luma-squared; d log(1+L) / dL = 1 / (1+L) converts its sigma so
    ...
    for (int Y = -2; Y <= 2; ++Y)
    {
        for (int X = -2; X <= 2; ++X)
        {
            ...
            // V1: clamp only STATISTICALLY UNCERTAIN brightness. A tap whose variance-of-the-mean says its value
            ...
            float TapLogLum       = log(1.0 + Luminance(TapColour.rgb));   // V4
    ...
    if (FinalLevel != 0u) { float D = DitherOffset(Pixel); imageStore(OutputImage, Pixel, vec4(ToneMap(Result.rgb * imageLoad(AlbedoImage, Pixel).rgb) + vec3(D, D, D), 1.0)); }   // V7+V5
}

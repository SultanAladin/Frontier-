...
// V5: +-half-LSB hash dither for the rgba8 presentation write. Smooth sky and fog gradients otherwise quantise
...
void main()
{
    ...
    if (Enabled == 0u || CentreSurface.w <= 0.0)
    {
        ...
        if (FinalLevel != 0u) { float D = DitherOffset(Pixel); imageStore(OutputImage, Pixel, vec4(ToneMap(Centre.rgb) + vec3(D, D, D), 1.0)); }   // V5
        return;
    }
    ...
    if (FinalLevel != 0u) { float D = DitherOffset(Pixel); imageStore(OutputImage, Pixel, vec4(ToneMap(Result.rgb) + vec3(D, D, D), 1.0)); }   // V5
}

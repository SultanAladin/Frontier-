...
void main()
{
    ...
    if (Enabled == 0u || CentreSurface.w <= 0.0)
    {
        ...
        if (FinalLevel != 0u) imageStore(OutputImage, Pixel, vec4(ToneMap(Centre.rgb * imageLoad(AlbedoImage, Pixel).rgb), 1.0));   // V7
        return;
    }
    ...
    if (FinalLevel != 0u) imageStore(OutputImage, Pixel, vec4(ToneMap(Result.rgb * imageLoad(AlbedoImage, Pixel).rgb), 1.0));   // V7
}

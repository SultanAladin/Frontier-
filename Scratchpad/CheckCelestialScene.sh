...
#      · CelestialTransliterationProof — the reference's own kernels, re-transcribed line-by-line out of
#        .ref_fs_snapshot.glsl into a GLSL-semantics shim and required to agree with the shipped C++ to the
#        bit. The physics proof catches a kernel that is wrong about the world; only this catches a kernel
#        that is wrong about the REFERENCE — a transposed argument or a drifted constant that still looks
...
# shellcheck disable=SC2086
${Compiler} ${CompilerFlags} -I. -o "${BuildDirectory}/CelestialTransliterationProof" \
    Scratchpad/CelestialTransliterationProof.cpp \
    Projects/Project-Zero/Source/CelestialIntegrator.cpp \
    DeviceExchange/OrientationClassifier.cpp || { echo "  transliteration proof failed to build"; exit 94; }
...
"${BuildDirectory}/CelestialTransliterationProof"
TransliterationFailures=$?
TotalFailures=$(( TotalFailures + TransliterationFailures ))
...
if [ "${TotalFailures}" -eq 0 ]; then
    echo "  CELESTIAL GATE PASSED — physics 0, transliteration 0, ground 0, combined 0, scene 0 failures"
else
    echo "  CELESTIAL GATE FAILED — physics ${PhysicsFailures}, transliteration ${TransliterationFailures}, ground ${GroundFailures}, combined ${CombinedFailures}, scene ${SceneFailures}"
fi
echo "──────────────────────────────────────────────────────────────────────────────────────────"
exit "${TotalFailures}"

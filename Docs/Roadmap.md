...
| 10 | Deferred material work (M4c dispersion, glints, displacement ch20, Tier-B multi-slab) | ⚠️ | 30 % | **Glints DONE and now VISIBLE** (§15/§15b, 2026-09-23): M6 read `slate_glint_*`; M6b replaced the sub-pixel sine field with a flake **lattice** — new `slate_glint_size` + `slate_glint_color_spread` (Slate2.zw, no offset moved), `uv_scale` = cells/m, chips replace the basecoat, per-flake pigment. Row 12 is automotive flake paint (aluminium \| tinted \| candy). Gated (`RunGlintSheet.sh`): arms differ 3.2 %, energy within 1.5 %, three pigments differ ≥ 10 %, zero out-of-range samples. Also fixed an M6 UB (the glint arrays were never seated for the level itself) and retired the duplicate sine-field flake model — the automotive preview runs the same lattice, so there is ONE flake model repo-wide. Left: M4c dispersion (spectral seam), displacement ch20 (tessellation), Tier-B multi-slab |

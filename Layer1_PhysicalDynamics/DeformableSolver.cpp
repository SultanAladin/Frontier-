//============================================================================================================================================
// 📦 Frontier/Layer1_PhysicalDynamics/DeformableSolver.cpp — XPBD Softbody and Cloth Solver Implementation
//============================================================================================================================================
...
DeformableSolver::DeformableSolver(TaskScheduler* Scheduler) noexcept
...
void DeformableSolver::UnregisterStructure(uint64_t StructureIdentifier) noexcept
{
    auto iterator = std::remove_if(m_Structures.begin(), m_Structures.end(),
        [StructureIdentifier](const DeformableStructure& s) { return s.StructureIdentifier == StructureIdentifier; });
    m_Structures.erase(iterator, m_Structures.end());
}
...
{
    if (m_Structures.empty() || Δτ <= 0.0f || SubstepCount == 0)
    {
        return;
    }
...
void DeformableSolver::SolveSubstep(float SubstepΔτ) noexcept
{
    for (auto& Structure : m_Structures)
    {
        ...
        for (auto& Particle : Structure.Particles)
        {
            ...
            Particle.Velocity += m_GravitationalAcceleration * SubstepΔτ;
...
const DeformableStructure* DeformableSolver::QueryStructure(uint64_t StructureIdentifier) const noexcept
{
    for (const auto& Structure : m_Structures)
    {
        if (Structure.StructureIdentifier == StructureIdentifier)
        {
            return &Structure;
        }
    }

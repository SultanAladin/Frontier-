...
foreach ($Src in $Sources)
{
    ...
    if ((Test-Path $Obj) -and
        ((Get-Item $Obj).LastWriteTimeUtc -gt (Get-Item $Src).LastWriteTimeUtc) -and
        ((Get-Item $Obj).LastWriteTimeUtc -gt (Get-Item $ConfigHPath).LastWriteTimeUtc))
    {
        continue
    }

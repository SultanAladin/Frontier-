...
export class Animator {
  constructor(skel, opts = {}) {
    ...
    this.idle = {
      ...
      jaw: 0, jawTarget: 0, nextJaw: 3, jawEvent: 0, breath: 0, breath2: 0, shift: 0,
      sniff: 0, sniffMode: 0, sniffDuration: 1.8, nextSniff: 4.5, sniffEvent: 0,
      // respiration: asymmetric (quicker inhale, longer exhale), depth wanders
      breathPhase: 0, breathPeriod: 4.2, breathDepth: 1, nextBreath: 6, breathEvent: 0,
      // event-driven weight shift: the animal periodically rests more of its
      // mass on one leg for a few seconds, then settles back to square.
      weight: 0, weightTarget: 0, weightUntil: 0, nextWeight: 4.5, weightEvent: 0,
      // ambient tail flick / adjust
      flick: 0, flickSide: 1, nextFlick: 7, flickEvent: 0,
      // slow head "alertness" that occasionally lifts the skull a little
      alert: 0, alertTarget: 0, nextAlert: 9, alertUntil: 0, alertEvent: 0,
    };
  ...
  _updateStep(dt) {
    ...
    // ---- 6-7. idle behaviour, then pelvis + axial (idle drives both)
    this._updateIdle(dt);
  ...
  _evalAction(dt) {
    ...
    if (a.type === 'bite') {
      // Feeding is staged like a real orthal (up-and-down) bite rather than one
      // body bend:
      //   anticipate → gape & gather → decisive downward strike → orthal jaw
      //   close → brief puncture press → puncture-and-pull with a head shake →
      //   settle with a small rebound. The mandible does the opening/closing;
      //   the neck + head drive the strike and the pull; the torso only braces.
      const v = a.variant ?? 0;
      const wind = ease(t / (v === 1 ? 0.38 : 0.34));
      // a short gather where the skull cocks back-and-up before it commits
      const antic = smoothstep(0, 0.16, t) * (1 - smoothstep(0.20, 0.42, t));
      const close = smoothstep(v === 1 ? 0.45 : 0.40, v === 2 ? 0.64 : 0.68, t);
      const contact = smoothstep(0.58, 0.73, t);
      // a brief clamp/press right after the jaws meet, before the pull begins
      const hold = smoothstep(0.66, 0.74, t) * (1 - smoothstep(0.84, 0.98, t));
      const pull = smoothstep(v === 2 ? 0.76 : 0.70, v === 1 ? 1.12 : 1.06, t)
        * (1 - smoothstep(1.08, v === 2 ? 1.50 : 1.42, t));
      const rec = ease((t - (v === 1 ? 1.28 : 1.20)) / 0.58);
      // a small elastic overshoot as the head returns to neutral
      const rebound = smoothstep(1.28, 1.48, t) * (1 - smoothstep(1.48, 1.74, t));
      ...
      A.pitch = (2.4 * wind - (v === 1 ? 3.8 : 4.8) * strike + 1.6 * pull - 1.2 * rebound) * DEG;
      A.drop = 0.10 * lowTarget * contact + 0.05 * lowTarget * hold + (a.hasTarget ? 0 : 0.13 * strike);
      // lunge/surge forward into the prey, press during the clamp, tug back on pull
      A.surge = 0.14 * wind + 0.10 * hold + (v === 1 ? 0.22 : v === 2 ? 0.28 : 0.18) * pull - 0.06 * antic;
      A.lungeW = a.hasTarget ? Math.max(strike, hold * 0.9) : 0;
      // Head: gather up on the anticipation, drive down through the strike, press
      // on the clamp, then lift/pull. Kept modest so the aimed tooth row still
      // lands on target (the aim solver owns the actual aiming).
      // NB: keep the head/neck pitch at the snap (t≈0.69) essentially equal to
      // the tuned aim, so the aimed tooth row still lands on target. The extra
      // "press" is expressed as translation (drop/surge) and only after the snap.
      const postPress = smoothstep(0.74, 0.86, t) * (1 - smoothstep(0.86, 1.0, t));
      A.headPitch = (6 * antic + 10 * wind - 2.5 * contact - 2.4 * postPress
        + (v === 2 ? 7 : v === 1 ? -3 : 2) * pull + 2.5 * rebound) * DEG;
      A.neckPitch = ((a.hasTarget ? 3 * wind : 3 * wind - 9 * strike) - 3.0 * antic - 1.6 * postPress
        + (v === 2 ? -4 : 3) * pull) * DEG;
      // Variant 1 rakes across the bite; variant 2 lifts the skull after the
      // puncture; variant 0 pulls straight back. All stay small additive motions
      // around the aimed tooth row, never a loose neck whip.
      A.headYaw += (v === 1 ? 7 : v === 2 ? -3 : 1) * DEG * pull;
      A.neckYaw += (v === 1 ? -4 : v === 2 ? 2 : 0) * DEG * pull;
      A.roll += (v === 1 ? 2.2 : v === 2 ? -0.8 : 0) * DEG * pull;
      // A subtle puncture-and-pull head shake on every bite (not only on a
      // registered hit): the clamped skull worries the wound as it draws back.
      {
        const sh = hold + pull;
        if (sh > 0.02) {
          const e = Math.sin(TAU * 2.6 * (t - 0.72)) * clamp(sh, 0, 1);
          A.headYaw += (v === 1 ? 3.5 : 2.4) * DEG * e;
          A.neckYaw += 1.6 * DEG * e;
          A.headRoll += (v === 2 ? 3.0 : 2.2) * DEG * e;
        }
      }
      // The mandible does almost all of the opening/closing. It is already shut
      // at the contact event, so the subsequent pull cannot read as a neck snap
      // or a floppy jaw. A faint re-grip chatter plays only after the snap.
      const gape = v === 1 ? 0.96 : v === 2 ? 1.10 : 1.03;
      const regrip = smoothstep(0.90, 0.98, t) * (1 - smoothstep(1.04, 1.16, t));
      A.jaw = gape * wind * (1 - close) + 0.035 * close * (1 - rec) + 0.06 * regrip * (1 - rec);
      A.aimW = a.hasTarget ? Math.max(wind * (1 - rec), contact) : 0;
      A.arms = Math.max(strike, hold) * 0.95;
      A.speedCap = t > 0.76 ? 2.0 : Infinity;
      if (a.hit) {
        // A registered catch adds a stronger, longer worry-shake on top.
        const st = t - 0.72;
        if (st > 0 && st < 0.42) {
  ...
  _updatePelvis(g, dt) {
    ...
    // idle.breath / idle.breath2 are shaped in _updateIdle (asymmetric respiration)
    ...
    // Resting weight-shift: the hips slide over the loaded leg, roll toward it
    // and settle a little lower — the classic "cocked hip" of a standing biped.
    const wt = idle.w * idle.weight;
    const y = SPEC.stance.acetabulumHeight - sm.crouch + bobRaw + idle.w * 0.014 * idle.breath
      - A.roar * 0.10 - A.drop - 0.035 * this.turnW - idle.w * Math.abs(idle.weight) * 0.020;
    P.position.set(swayRaw + idle.w * idle.shift + wt * 0.11 + A.sideShift + pivotShift, y, surgeRaw + A.surge);
    ...
    _e.set(pitch, yaw, rollRaw - idle.w * idle.shift * 0.6 - wt * 2.6 * DEG + A.roll + pivotRoll + this.turnBank, 'YXZ');
  ...
  _updateIdle(dt) {
    ...
    // ---- respiration -------------------------------------------------------
    // A shaped breath (rise on the inhale, longer settle on the exhale) reads as
    // a living animal rather than a metronome. Every few cycles it takes a
    // deeper, slower breath. Two harmonics let the ribcage and the flanks move
    // slightly out of phase.
    if (t > I.nextBreath) {
      const e = I.breathEvent++;
      const deep = hash01(e + 90) < 0.34;
      I.breathPeriod = deep ? 5.4 + hash01(e + 91) * 1.6 : 3.7 + hash01(e + 92) * 1.1;
      I.breathDepth = deep ? 1.35 + hash01(e + 93) * 0.5 : 0.85 + hash01(e + 94) * 0.35;
      I.nextBreath = t + I.breathPeriod * (deep ? 1 : 2 + Math.floor(hash01(e + 95) * 3));
    }
    I.breathPhase = (I.breathPhase + dt / Math.max(0.5, I.breathPeriod)) % 1;
    {
      const bp = I.breathPhase;
      // 0..1: quick inhale (rise) then a longer exhale (fall)
      const shaped = bp < 0.40 ? smoothstep(0, 0.40, bp) : 1 - smoothstep(0.40, 1.0, bp);
      I.breath += ((shaped * 2 - 1) * I.breathDepth - I.breath) * (1 - Math.exp(-dt * 6));
      const bp2 = (bp + 0.12) % 1;
      const shaped2 = bp2 < 0.40 ? smoothstep(0, 0.40, bp2) : 1 - smoothstep(0.40, 1.0, bp2);
      I.breath2 += ((shaped2 * 2 - 1) * I.breathDepth - I.breath2) * (1 - Math.exp(-dt * 6));
    }

    // ---- weight shift ------------------------------------------------------
    if (I.weightUntil !== 0 && t > I.weightUntil) {
      I.weightTarget = 0;                       // return to square
      I.weightUntil = 0;
      I.nextWeight = t + 2.0 + hash01(I.weightEvent + 20) * 3.5;
    } else if (I.weightUntil === 0 && t > I.nextWeight) {
      const e = I.weightEvent++;
      I.weightTarget = (hash01(e + 21) < 0.5 ? -1 : 1) * (0.45 + hash01(e + 22) * 0.55);
      I.weightUntil = t + 2.4 + hash01(e + 23) * 3.4;
      I.nextWeight = Infinity;
    }
    I.weight += (I.weightTarget - I.weight) * (1 - Math.exp(-dt * 1.6));

    // ---- ambient tail flick ------------------------------------------------
    if (I.flick <= 0 && t > I.nextFlick) {
      const e = I.flickEvent++;
      I.flickSide = hash01(e + 40) < 0.5 ? -1 : 1;
      I.flick = 1;
      I.nextFlick = t + 4.5 + hash01(e + 41) * 7.0;
    }
    I.flick = Math.max(0, I.flick - dt / 0.9);

    // ---- occasional alert head-lift ---------------------------------------
    if (I.alertUntil !== 0 && t > I.alertUntil) {
      I.alertTarget = 0; I.alertUntil = 0;
      I.nextAlert = t + 5 + hash01(I.alertEvent + 60) * 8;
    } else if (I.alertUntil === 0 && t > I.nextAlert) {
      const e = I.alertEvent++;
      I.alertTarget = 0.5 + hash01(e + 61) * 0.5;
      I.alertUntil = t + 1.4 + hash01(e + 62) * 2.2;
      I.nextAlert = Infinity;
    }
    I.alert += (I.alertTarget - I.alert) * (1 - Math.exp(-dt * (I.alertTarget ? 3.2 : 1.8)));
  ...
  _updateAxial(g, dt) {
    ...
    for (let i = 0; i < nT; i++) {
      ...
      // breathing bows the dorsal column very slightly (ribcage lift on inhale),
      // strongest over the mid-back; a second harmonic gives the flanks life.
      const pitch = -0.08 * DEG * I.breath * I.w - A.roar * 1.0 * DEG * u
        + 0.055 * DEG * I.breath2 * I.w * Math.sin(u * Math.PI);
    ...
    for (let i = 0; i < nC; i++) {
      ...
      // ambient flick: a quick out-and-back that travels to the tip
      if (I.flick > 0) {
        const fe = Math.sin(Math.PI * (1 - I.flick));           // 0→1→0 envelope
        lat += I.w * I.flickSide * fe * (1.0 + 3.2 * u) * DEG * Math.max(0, u - 0.1);
      }
    ...
    // ambient idle head drivers (shared by neck + skull so the lift is supported)
    const idleFree = I.w * (1 - noLook);
    const driftYaw = noise1(this.time * 0.11 + 3, 7) * 3.4 * DEG * idleFree;
    const driftPitch = noise1(this.time * 0.09 + 8, 11) * 2.2 * DEG * idleFree;
    const alertLift = I.alert * 7.5 * DEG * idleFree;   // raise the snout to scan
    ...
    for (let i = 0; i < nN; i++) {
      ...
      const pitch = (lookPitch * 0.55 + cancelPitch + sniffPitch + A.neckPitch) * w / nN + this.aimPitch * 0.85 * wa
        ...
        + (alertLift * 0.45 + driftPitch * 0.35) * w / nN;   // neck supports the idle head motion
    ...
    {
      const h = S.head;
      // Ambient life when the head is neither aiming nor executing a scripted
      // look: a slow wandering micro-drift plus an occasional alert lift of the
      // skull (drivers hoisted above the neck loop so the neck supports it).
      const pitch = lookPitch * 0.35 + sniffPitch * 0.8 + A.roar * 12 * DEG + A.headPitch
        + this.aimPitch * 0.15 + driftPitch + alertLift;
      const yaw = lookYaw * 0.58 + A.headYaw + this.aimYaw * 0.15 + driftYaw;
      const roll = I.look.z * DEG * I.w * 0.8 * (1 - noLook) + A.headRoll
        + noise1(this.time * 0.07 + 20, 5) * 1.2 * DEG * idleFree;

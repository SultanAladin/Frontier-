...
import { buildSkeleton, LEG } from '../js/skeleton.js';
import { Animator } from '../js/animator.js';
import { buildDecorations } from '../js/decorations.js';
import { Hunter } from '../js/hunter.js';
import { gaitAt, GAIT_KEYS, clamp, WALK_SPEED, RUN_SPEED } from '../js/gait.js';
import { SPEC } from '../js/spec.js';
...
console.log('\nRealistic build: no sci-fi hardware, bite variants and hunt policy');
{
  const [dSk, dAnim] = mk();
  dAnim.update(1 / 60);
  const decorations = buildDecorations(dSk);
  ok(decorations.group.children.length === 0, 'no alien antennae / hydraulic actuators / cables are attached (clean skeleton)');
  let hardware = 0;
  dSk.rig.traverse((o) => { if (o.userData && o.userData.decoration) hardware++; });
  ok(hardware === 0, 'no decoration-tagged meshes remain anywhere on the rig');

...
bool Tier2EqualSlab(const MaterialSlabDescriptor& A, const MaterialSlabDescriptor& B, std::string& Field)
{
    ...
    if (X.SlateGlintSize != Y.SlateGlintSize) { Field = "glint-size"; return false; }                      // M6b
    if (X.SlateGlintColorSpread != Y.SlateGlintColorSpread) { Field = "glint-color-spread"; return false; }
...
void T24_ExtrasFlat()
{
    ...
    S.SlateGlintSize = 0.375f; S.SlateGlintColorSpread = 0.75f;   // M6b: the sized/coloured pair rides extras too
    const std::string J1 = EncodeIdentity(D);

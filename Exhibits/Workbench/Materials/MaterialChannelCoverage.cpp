...
Frontier::MaterialSlabDescriptor KitchenSink() noexcept
{
    ...
    S.SlateAnisotropyRotation = 0.7f;   // M2 (SlateDirectF0Weight stays 0: still reserved)
    S.SlateGlintSize = 0.33f; S.SlateGlintColorSpread = 0.8f;   // M6b: Slate2.zw
...
void ProofRecordMirror()
{
    ...
    CHECK(S.DirectF0Weight == 0.0f, "direct-f0 lane still reserved (0 = IOR-derived F0)");
    // M6b claimed the last two lanes of Slate2 for the flake pair. The offsets below are what pins them.
    CHECK(S.GlintSize == 0.33f && S.GlintColorSpread == 0.8f, "M6b glint size / colour spread reach the record");
    CHECK(offsetof(MaterialSlabRecord, GlintSize) == 296u && offsetof(MaterialSlabRecord, GlintColorSpread) == 300u,
          "M6b lanes are Slate2.zw — nothing renumbered");

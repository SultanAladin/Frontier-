...
### 15b. M6b — the flakes get a SIZE and a COLOUR (2026-09-23, roadmap #10)

§15's gates were green and the picture was still wrong. Desk report: *"the flakes are visible but they are
too small."* That is not a tuning complaint, it is a modelling one, and the measurement agrees.

**Why the sine field could not be tuned into flakes.** `AutomotiveFlakeSignal` is a sum of three incommensurate
sines thresholded into a sparkle. Its features have no independent size: raising `uv_scale` makes them finer,
lowering it makes the sparkle vanish into broad blotches, and at the library's 4 → 12 (× 3) every feature was
below a pixel. A field also has no per-feature identity, so it cannot give one flake one orientation, and it
certainly cannot give one flake its own pigment. A real flake has three authorable properties a noise field
cannot express: a position, a **radius**, and one constant orientation and colour across its whole face.

**The lattice** (`Engine/Shaders/AutomotiveMaterialProfiles.slang` — `SlateFlakeSample` / `SlateFlakeNormal`,
shared text run by the kernel and the CPU mirror). Quantise the surface point into cells of 1/`uv_scale`
metres; each cell hosts at most one flake, seeded from its own hash. One cell is inspected per hit (the
jitter is bounded so a flake never crosses a wall) — a handful of hashes, no neighbourhood search.

- `slate_glint_uv_scale` is now the **lattice frequency in cells per metre**, read directly; the ×3 fudge is gone.
- **`slate_glint_size`** (new) — the chip's radius as a fraction of its cell (0.02 … 0.5). *This is the knob
  the desk was missing*: it is what makes a flake wider than a pixel at a sane cell frequency.
- **`slate_glint_color_spread`** (new) — each chip draws its own pigment off a hue wheel. Automotive
  multi-colour candy flake; a single-tint field cannot do this at all.

Both new floats ride the slab record's two trailing **reserved** lanes (`Slate2.zw`), so nothing in the
58-float prefix and no pinned offset moved (`MaterialChannelCoverage` now asserts 296/300 explicitly).

**Two corrections the pictures forced, both recorded because both were wrong first:**

1. *The chip REPLACES the basecoat, it does not tint it.* Multiplying the pigment into the base colour is
   what a coloured filter does, and it rendered every flake **darker** than the dark paint around it — the
   exact opposite of a flake. A flake is a chip of its own metal lying on paint: at mask 1 the surface is
   that metal, at 0 it is the paint, and the rim feathers between.
2. *The pigment wheel needs a floor (`kFlakeFloor`), and the floor is MEASURED.* A hue at full saturation has
   exact zeros in one or two channels; a metal whose F82 tint is zero in a channel drives that channel's
   Kulla–Conty energy-compensation term to a tiny **negative** radiance. Candy column, 360×270×24 spp:
   floor 0.00 → **6 617** out-of-range samples, 0.06 → **1 344**, 0.15 → **91**, **0.22 → 0** (shipped). The
   aluminium and tinted columns, which never reach a zero channel, reported 0 throughout. A real pigment
   flake is not a perfect notch filter either, so the floor is physical as well as numerical.

**A real bug found on the way.** §15's mirror seated the glint arrays in the interface-panel path ONLY, so on
the level itself every `g_MatGlintDensity[Material]` read past the end of an **empty** vector — undefined
behaviour that happened not to crash, and whatever it read is what the first glint sheet actually rendered.
The level's materials are now seated from their own records before any shading runs, with a bounds guard.

**Row 12 is now the material the model exists for**: dark saturated basecoat under a clear coat, lattice
190 → 70 cells/m, chip 0.26 → 0.40 of a cell, and colour spread in **thirds** across the columns — cols 0-4
plain aluminium, 5-9 a tinted mix, 10-14 full candy flake, so one shot shows all three pigments.

**Gates** (`RunGlintSheet.sh`, full 640×360×48 + three 480×360 close-ups):

| Gate | Claim | Measured |
|---|---|---|
| ① arms differ | the pair is READ | RMSE **0.0323** vs the 0.012 floor |
| ② energy | a flake moves light, never makes it | means 0.1027 vs 0.1012 (**1.5 %**, limit 2 %) |
| ③ pigments differ | `color_spread` is READ | **0.103 / 0.109 / 0.102** pairwise vs the 0.01 floor |
| ④ finite film | the `kFlakeFloor` measurement holds | **0** out-of-range samples, every arm |

The row A/B also moved: the old `glints` eye (Y = 12.7) put two rows of other materials in front of the
subject, which is half of why the first sheet looked unchanged. Both sheets now shoot from the 11/12 gap.

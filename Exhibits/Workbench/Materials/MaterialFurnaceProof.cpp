...
// M6b — the flake lattice. This is shared shader text that BOTH the kernel and the CPU mirror run, and it is the
//    only part of the material model whose correctness is geometric rather than radiometric, so it is pinned here
//    rather than only in a picture. Five claims, each one a bug that was actually possible:
//      ① determinism      — the same world point returns the same flake (the GPU/CPU mirror rests on this)
//      ② containment      — a flake never leaves its cell, which is what licenses the ONE-cell lookup
//      ③ coverage         — the fraction of cells holding a flake tracks the requested coverage
//      ④ size is a SIZE   — a larger slate_glint_size covers strictly more surface at a fixed lattice
//      ⑤ no zero channel  — the pigment wheel's kFlakeFloor holds on every hue (the measured negative-radiance bug)
void ProofFlakeLattice()
{
    std::printf("[furnace] M6b flake lattice\n");

    // ① determinism: same point, same answer; and the lattice actually varies in space.
    const vec3 Probe(0.317f, -1.244f, 0.585f);
    const vec4 A = SlateFlakeSample(Probe, 60.0f, 0.8f, 0.35f, 1.0f);
    const vec4 B = SlateFlakeSample(Probe, 60.0f, 0.8f, 0.35f, 1.0f);
    CHECK(A.x == B.x && A.y == B.y && A.z == B.z && A.w == B.w, "flake sample is deterministic");

    // ② containment + ③ coverage + ④ monotone size, measured on a dense lattice sweep. Each sample walks a fixed
    //    grid of points in one metre cubed; at 60 cells/m that is many cells, so the ratios are stable.
    const auto Measure = [](float coverage, float size)
    {
        int Hit = 0, Total = 0;
        for (int I = 0; I < 40; ++I)
            for (int J = 0; J < 40; ++J)
                for (int K = 0; K < 8; ++K)
                {
                    const vec3 P(static_cast<float>(I) * 0.0253f, static_cast<float>(J) * 0.0251f,
                                 static_cast<float>(K) * 0.0257f);
                    if (SlateFlakeSample(P, 60.0f, coverage, size, 1.0f).x > 0.0f) ++Hit;
                    ++Total;
                }
        return static_cast<float>(Hit) / static_cast<float>(Total);
    };
    const float Sparse = Measure(0.15f, 0.35f);
    const float Dense  = Measure(0.90f, 0.35f);
    CHECK(Sparse < Dense, "coverage is read: 0.15 -> %.3f of the surface, 0.90 -> %.3f", Sparse, Dense);
    CHECK(Sparse < 0.15f + 0.02f, "a 0.15-coverage lattice cannot cover more surface than its cells (%.3f)", Sparse);

    const float Small = Measure(0.90f, 0.10f);
    const float Big   = Measure(0.90f, 0.45f);
    CHECK(Small < Big, "slate_glint_size is a SIZE: 0.10 -> %.3f of the surface, 0.45 -> %.3f", Small, Big);

    // ② containment, stated exactly: the flake's mask must be 0 on every cell WALL, or one cell's chip would bleed
    //    into its neighbour and the single-cell lookup would miss half of it. Walk the six faces of many cells.
    int Bleed = 0;
    for (int I = 0; I < 12; ++I)
        for (int J = 0; J < 12; ++J)
            for (int Face = 0; Face < 6; ++Face)
            {
                // A point ON a cell boundary (a whole multiple of the cell size in one axis).
                const float U = static_cast<float>(I) * 0.0713f, V = static_cast<float>(J) * 0.0917f;
                vec3 P(U, V, 0.0f);
                const float Cell = 1.0f / 60.0f;
                const float Wall = std::floor(U / Cell) * Cell;   // exactly on the wall
                if (Face == 0) P = vec3(Wall, V, 0.0f);
                else if (Face == 1) P = vec3(Wall, V, U);
                else if (Face == 2) P = vec3(V, Wall, U);
                else if (Face == 3) P = vec3(U, Wall, V);
                else if (Face == 4) P = vec3(U, V, Wall);
                else P = vec3(V, U, Wall);
                if (SlateFlakeSample(P, 60.0f, 1.0f, 0.5f, 1.0f).x > 0.0f) ++Bleed;
            }
    CHECK(Bleed == 0, "no flake reaches a cell wall at the maximum size (%d bleeds) — the one-cell lookup is exact", Bleed);

    // ⑤ the pigment floor. A hue with an exact zero channel drove the Kulla-Conty compensation term to a small
    //    NEGATIVE radiance (measured: 6 617 out-of-range samples on the candy column at floor 0). Every hue on the
    //    wheel must keep every channel strictly positive, and must stay inside [0, 1].
    float MinChannel = 1.0f, MaxChannel = 0.0f;
    for (int I = 0; I < 720; ++I)   // the whole wheel, summarised into two checks (720 lines of 'ok' help nobody)
    {
        const vec3 H = SlateFlakeHue(static_cast<float>(I) / 720.0f);
        MinChannel = std::fmin(MinChannel, std::fmin(H.x, std::fmin(H.y, H.z)));
        MaxChannel = std::fmax(MaxChannel, std::fmax(H.x, std::fmax(H.y, H.z)));
    }
    CHECK(MaxChannel <= 1.0f, "every flake pigment on the wheel stays in gamut (max channel %.4f)", MaxChannel);
    CHECK(MinChannel > 0.0f, "no flake pigment has a zero channel (min %.4f) — the negative-radiance bug", MinChannel);
    CHECK(MinChannel >= 0.20f, "the measured kFlakeFloor (0.22) is intact (min channel %.4f)", MinChannel);

    // The facet is a NORMAL: unit length, and the bare-paint case (mask 0) must return the surface untouched.
    const vec3 N(0.0f, 0.0f, 1.0f);
    const vec3 Bare = SlateFlakeNormal(N, Probe, 60.0f, 0.0f, 0.35f);
    CHECK(Bare.x == N.x && Bare.y == N.y && Bare.z == N.z, "mask 0 leaves the shading normal alone");
    const vec3 Facet = SlateFlakeNormal(N, Probe, 60.0f, 1.0f, 0.35f);
    CHECK(std::fabs(length(Facet) - 1.0f) < 1e-5f, "the flake facet is unit length (%.6f)", length(Facet));
}
